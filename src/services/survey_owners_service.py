...
class SurveyOwnersService:
    ...
    def add_owner_to_survey(self, survey_id, user_email):
        ...
        if not survey:
            message = "Kyselyä ei löydy järjestelmästä"
        ...
        if exists:
            message = gettext("Käyttäjällä on jo oikeudet kyselyyn!")
            return (False, message)
        ...
        if not success:
            message = gettext("Hallintaoikeuksien lisääminen käyttäjälle epäonnistui")
            return (False, message)
