...
function cellIsValid(elem) {
  ...
  var colHeader = document.querySelector(
    `#choice-table-headers th:nth-child(${cellIndex + 1})`
  );
...
function removeValidationErrorMsg(elem) {
  ...
  var warningTextElement = document.querySelector(
...
function createNewSurvey() {
  ...
  var elementsToValidate = document.querySelectorAll(
    "input[validation-regex]:not(.hidden)"
  );
  ...
  var tableHeaders = Array.from(
    document.querySelectorAll("#choice-table-headers th:not(:last-of-type)")
  ...
  var requestData = {
    ...
    minchoices: minChoicesElement.classList.contains("hidden")
      ? rowsAsJson.length
      : Number(minChoicesElement.value),
    allowedDeniedChoices: allowedDeniedChoices.classList.contains("hidden")
      ? 0
      : Number(allowedDeniedChoices.value),
    allowSearchVisibility: document.getElementById("search-visibility-yes")
      .checked,
  };
  ...
  $.ajax({
    ...
    error: function (result) {
      if (result.responseJSON) {
        showAlert({
          msg: result.status + ": " + result.responseJSON.msg,
          color: "red",
        });
      } else {
        showAlert({
          msg: `Jokin meni vikaan, palvelimeen ei saatu yhteyttä`,
          color: "red",
        });
      }
    },
...
function showDeleteColumnIconOnHover(event) {
  ...
  if (columnIndex > 2) {
    var columnDeleteBtn = document.querySelector(
      `#column-delete-btns td:nth-child(${columnIndex + 1})`
    );
    columnDeleteBtn.classList.add("visible");
    columnDeleteBtn
      .querySelector(".delete-col-btn")
      .classList.add("delete-col-btn-visible");
  }
}
...
function removeColumn(event) {
  ...
  var rows = document
    .querySelector(".choice-table-main")
    .querySelectorAll("tr");
...
function createAddColumnHeader() {
  var newAddColumnHeader = createElementWithText(
    "th",
    "+ Lisää tietokenttä",
    editCell
  );
...
function editCell(event) {
  ...
  if (
    event.target.id !== "add-column-header" &&
...
function submitNewColumn(event) {
  ...
  document
    .querySelector("#column-delete-btns")
    .appendChild(createDeleteColumnCell());

  if (!document.getElementById("add-column-header")) {
    // -->: Create new "Add new column" header to the table if it does not exist anymore
    document
      .getElementById("choice-table-headers")
      .appendChild(createAddColumnHeader());
...
function pageLoadActions() {
  ...
  var variableHeaders = document.querySelectorAll(
    "#choice-table-headers th:not(.constant-header, #add-column-header)"
  );
  ...
  if (event.target.value === "show") {
    Array.from(
      document.querySelectorAll(
        `.${event.target.getAttribute("data-dependant-class")}`
      )
    ).forEach((element) => {
...
function setUploadedTableValues(table) {
  ...
  var variableHeaders = Object.keys(table[0]).filter(
    (header) => !["name", "spaces", "min_size", "Mandatory"].includes(header)
  );
  variableHeaders.forEach((header) => {
    var th = document.createElement("th");
    th.innerText = header;
  ...
  table.forEach((row) => {
    ...
    if (mandatoryValue === "true") {
      checkboxCell.innerHTML =
        '<input type="checkbox" name="choice-checkbox" checked="checked">';
    } else {
      checkboxCell.innerHTML = '<input type="checkbox" name="choice-checkbox">';
    }
    ...
    addCellEventListeners(
      rowElement.appendChild(createElementWithText("td", row["min_size"]))
    );
...
function enableSelectAllCheckbox(
  selectAllId = "select-all-choices",
  checkboxName = "choice-checkbox"
) {
