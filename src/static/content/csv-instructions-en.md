...
In this case, the separator in the CSV-file does not matter (it can be , or ; etc.). The file must at minimun contain the name, capacity and minimum group size for each choice. The first row of the file must be a header row.</br>
...
## When using E-lomake
When creating a survey through E-lomake, the first field must be the name of the choice, the second field must be the amount of seats available in the choice and the third field must be the minimum size of seats for a choice. If any of these is missing, importing the CSV file will not work. 
...
In the first three fields, the name does not matter (Daycare name, name, ID, etc., or capacity, maximum capacity, etc., are all fine, and min_size, minimum size, etc.), as long as the order is correct.

Responding to the above survey will produce a CSV file, which can be found in the Form Report section of E-lomake.
