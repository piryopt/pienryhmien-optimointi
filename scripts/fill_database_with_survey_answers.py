...
def add_teacher_for_survey(survey_id):
    ...
    db.session.execute(
        text("INSERT INTO survey_owners (survey_id, user_id)VALUES (:survey_id, :user_id)"), {"survey_id": survey_id, "user_id": teacher_id}
    )
...
def add_users_to_db(student_amount):
    db.session.bulk_insert_mappings(
        User,
        [
            {"id": i, "name": f"user_{i}", "email": f"user_{i}@mail.com", "isteacher": False, "admin": False, "language": "fi"}
            for i in range(1, student_amount + 1)
        ],
    )
...
def add_groups_to_db(group_amount, survey_id):
    db.session.bulk_insert_mappings(
        SurveyChoice,
        [
            {"id": i, "survey_id": survey_id, "name": f"group_{i}", "max_spaces": 100, "deleted": False, "min_size": 1, "mandatory": False}
